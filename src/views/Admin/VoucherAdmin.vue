            ...
            <v-data-table :headers="headers" :items-per-page="10" :items="getListData" item-value="id" class="elevation-10">
                <template v-slot:item="{ item }">
                    <tr>
                        ...
                        <td class="border-right border-bot">{{ item.discount }}</td>
                        <td class="border-right border-bot">{{ item.expired_Time.split('T')[0] }}</td>
...
export default {
    ...
    computed: {
        ...mapGetters('voucherModules', ['getListData']),
    },
    created() {
        this.fetchListData();
    },
    data() {
        return {
            ...
            headers: [
                { title: 'STT', key: 'id' },
                { title: 'Tên voucher', key: 'name' },
                { title: 'Phần trăm giảm giá', key: 'discount' },
                { title: 'Ngày hết hạn', key: 'expired_Time' },
    ...
    methods: {
        ...
        ...mapActions('voucherModules', ['createObj', 'fetchListData', 'updateObj','deleteObj']),
    }
};
</script>
<style scoped>
